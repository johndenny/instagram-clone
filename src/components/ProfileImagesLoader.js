...
const ProfileImagesLoader = (props) => {
  ...
  const likeHandler = async (index) => {
    ...
    if (alreadyLiked === -1) {
      const newIsButtonLiked = [...isButtonLiked];
      newIsButtonLiked.splice(index, 1, true);
      setIsButtonLiked(newIsButtonLiked);
      ...
      const newIsButtonLiked = [...isButtonLiked];
      newIsButtonLiked.splice(index, 1, false);
      setIsButtonLiked(newIsButtonLiked);
      const newLikes = [...photosArray[index][0].likes];
      const newPost = {...photosArray[index][0], likes: newLikes};
      const newArray = [...photosArray[index]];
  ...
  useEffect(() => {
    ...
    setIsButtonLiked(new Array(12).fill(false));
  },[]);
                ...
                <footer className='feed-post-footer'>
                  <div className='feed-footer-buttons-wrapper'>
                    ...
                    >
                      ...
                      {liked !== -1 &&
                        <svg aria-label="Unlike" className={isButtonLiked[postIndex] ? "like-filled-svg animated" : 'like-filled-svg'} color="#ed4956" fill="#ed4956" height="24" role="img" viewBox="0 0 48 48" width="24">
